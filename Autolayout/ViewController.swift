...
class ViewController: UIViewController, UITextFieldDelegate
{
    ...
    private func updateUI() {
        ...
        if let lastLogin = loggedInUser?.lastLogin {
            ...
            let time = dateFormatter.string(from: lastLogin)
            
            let numberFormatter = NumberFormatter()
            numberFormatter.maximumFractionDigits = 1
            let daysAgo = numberFormatter.string(from: NSNumber(value: -lastLogin.timeIntervalSinceNow / (60 * 60 * 24)))
            let lastLoginFormatString = NSLocalizedString("Last Login %@ days ago at %@", comment: "Reports the number of days ago and time that user last logged in")
    ...
    private struct AlertStrings {
        struct LoginError {
            static let Title = NSLocalizedString("Login Error", comment: "Title of alert when user types in an incorrect user name or password")
                        static let Message = NSLocalizedString("Invalid user name or password", comment: "Message in alert when user types in an incorrect user name or password")
            static let DismissButton = NSLocalizedString("Try Again", comment: "The only button available in an alert presented when user types in an incorrect user name or password")
        }
    }
